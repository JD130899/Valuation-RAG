# app.py  (drop-in replacement with anti-flicker rendering)
...
# ================= Chat UI helpers =================
def type_bubble(text: str, *, base_delay: float = 0.012, cutoff_chars: int = 2000, target=None):
    # stable typewriter that only updates innerHTML of a fixed placeholder
    placeholder = target or st.empty()
    buf = []
    count = 0
    for ch in text:
...
def _reset_chat():
    st.session_state.messages = [
        {"id": "m0", "role": "assistant", "content": "Hi! I am here to answer any questions you may have about your valuation report."},
        {"id": "m1", "role": "assistant", "content": "What can I help you with?"}
    ]
...
if "next_msg_id" not in st.session_state:
    st.session_state.next_msg_id = 2
if "rendered_refs" not in st.session_state:
    st.session_state.rendered_refs = set()
...
/* --- ANTI-FLICKER ADDITIONS --- */
/* Remove all CSS transitions/animations that cause micro-flicker */
*, *:before, *:after { transition: none !important; animation: none !important; }
/* Prevent dimming during disabled/rerun states */
button[kind], div[role="button"], button[disabled] { opacity: 1 !important; filter: none !important; }
/* Keep headers/layout from briefly reflowing */
div[data-testid="stAppViewContainer"] .block-container { will-change: auto !important; }
/* Stop spinner/status fade-in flashes */
div[role="status"], [data-testid="stStatusWidget"] { transition: none !important; }
</style>
""", unsafe_allow_html=True)
...
def fix_markdown_tables(text: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if is_row:
            ...
            if all(dash_re.fullmatch(c) for c in cells): continue
    ...
    if st.sidebar.button("Load selected PDF"):
        ...
        if chosen["id"] == st.session_state.get("last_synced_file_id"):
            ...
            if path:
                ...
                _reset_chat()
                st.session_state.rendered_refs = set()  # reset rendered refs for new file

# ================= Fragments (optional, safe no-ops if not supported) =================
try:
    fragment = st.fragment
except AttributeError:
    fragment = getattr(st, "experimental_fragment", lambda f: f)
...
@fragment
def render_file_badge(name: str, pdf_bytes: bytes, synced: bool = True):
    file_badge_link(name, pdf_bytes, synced)
...
# ============== File input / badge (no flicker: only when changes) ==============
if "uploaded_file_from_drive" in st.session_state:
    render_file_badge(st.session_state.uploaded_file_name, st.session_state.uploaded_file_from_drive, True)
    up = io.BytesIO(st.session_state.uploaded_file_from_drive)
    up.name = st.session_state.uploaded_file_name
else:
    up = st.file_uploader("Upload a valuation report PDF", type="pdf")
    if up:
        render_file_badge(up.name, up.getvalue(), False)
...
# ========================== STABLE CHAT RENDERERS ==========================
def _render_bubbles(messages: list) -> str:
    parts = []
    for msg in messages:
        cls = "user-bubble" if msg["role"] == "user" else "assistant-bubble"
        parts.append(f"<div class='{cls} clearfix'>{msg['content']}</div>")
    return "\n".join(parts)

# Fixed, long-lived placeholders (pre-mounted; we only update innerHTML)
chat_container = st.empty()
thinking_container = st.empty()

# Initial paint of chat (single DOM mount)
chat_container.markdown(_render_bubbles(st.session_state.messages), unsafe_allow_html=True)
...
# Chat input (does not reflow the page)
user_q = st.chat_input("Type your question here…", key="main_chat_input")
if user_q:
    queue_question(user_q)

# ========================== ANSWER (no new containers; update placeholders) ==========================
if st.session_state.waiting_for_response and st.session_state.pending_input:
    thinking_container.markdown("<div class='assistant-bubble clearfix'>Thinking…</div>", unsafe_allow_html=True)
    ...
    if is_deny:
        ...
        # Clear thinking + re-render chat blob once
        thinking_container.markdown("", unsafe_allow_html=True)
        chat_container.markdown(_render_bubbles(st.session_state.messages), unsafe_allow_html=True)
...
""",
        ...
        # Clear thinking without removing node
        thinking_container.markdown("", unsafe_allow_html=True)

        # Append message to state first
        st.session_state.messages.append(entry)

        # Repaint the single chat blob (one DOM update)
        chat_container.markdown(_render_bubbles(st.session_state.messages), unsafe_allow_html=True)

        # Render ONE reference card only once per assistant message (no loops over history)
        ...
        if docs and not skip_reference:
            try:
                ...
                if best_doc is not None:
                    ...
                    # Only render if not already rendered this entry
                    if entry["id"] not in st.session_state.rendered_refs:
                        ...
                        st.session_state.rendered_refs.add(entry["id"])
...
# If some historical messages already had source info (e.g., from an earlier run),
# render their cards once and remember, without looping every rerun.
for msg in st.session_state.messages:
    if msg.get("source_img") and msg.get("source_pdf_b64") and msg.get("source_page"):
        mid = msg.get("id", "")
        if mid and mid not in st.session_state.rendered_refs:
            render_reference_card(
            ...
            st.session_state.rendered_refs.add(mid)

# Keep-alive ping (no visual impact)
components.html(
    """
    <script>
