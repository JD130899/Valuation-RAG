Now, another last task
the code which u just gave, Can we take this exact code for buttons, and put in my existing code, like replace the button code which exists there with this updated one..and rest everyhting remains same..i alreadqy have the rag flow there, u can see it 
...
for m in st.session_state.messages:
    if m["role"] == "user":
        st.markdown(f"<div class='chat-row' style='justify-content: flex-end'><div class='user-bubble'>{m['content']}</div></div>", unsafe_allow_html=True)
    else:
        st.markdown(f"<div class='chat-row' style='justify-content: flex-start'><div class='assistant-bubble'>{m['content']}</div></div>", unsafe_allow_html=True)
...
if user_text:
    ...
    st.markdown(f"<div class='chat-row' style='justify-content: flex-end'><div class='user-bubble'>{user_text}</div></div>", unsafe_allow_html=True)

    # assistant placeholder
    thinking_slot = st.empty()
    with thinking_slot.container():
        st.markdown("<div class='chat-row' style='justify-content: flex-start'><div class='assistant-bubble'>🧠 Thinking…</div></div>", unsafe_allow_html=True)

    # call LLM
    reply = call_llm(user_text)

    # replace thinking with final reply
    thinking_slot.markdown(f"<div class='chat-row' style='justify-content: flex-start'><div class='assistant-bubble'>{reply}</div></div>", unsafe_allow_html=True)
